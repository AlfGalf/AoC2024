    ...
    for (total, parts) in input.clone() {
        fn iter(target: usize, cur: usize, mut next: impl Clone + Iterator<Item = usize>) -> bool {
            if let Some(num) = next.next() {
                if iter(target, cur + num, next.clone()) {
                    true
                } else if iter(target, cur * num, next) {
                    true
                } else {
                    false
                }
    ...
    for (total, parts) in input {
        fn iter(target: usize, cur: usize, mut next: impl Clone + Iterator<Item = usize>) -> bool {
            if let Some(num) = next.next() {
                if cur > target {
                    false
                } else if iter(target, cur + num, next.clone()) {
                    true
                } else if iter(target, cur * num, next.clone()) {
                    true
                } else if iter(target, format!("{cur}{num}").parse().unwrap(), next) {
                    true
                } else {
                    false
                }
